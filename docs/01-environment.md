...
### なぜこの向きなのか：デバッガと IDE の関係

そもそもデバッグは **「IDE(画面)」と「デバッガ本体(エンジン)」** の2つに分かれています。
IDE 自体はデバッグしていません。実行を止めて変数を覗くのは、プログラムに組み込まれた
**デバッガエンジン（PHP では Xdebug）**で、IDE はそれに指示を出す**画面**です。両者は
**通信プロトコル（Xdebug の場合は DBGp、TCPソケット）**でつながっています。

この「IDE ⇄ プロトコル ⇄ エンジン」という構造は **PHP に限らずどの言語でも同じ**です
（Python は debugpy、JS は V8 Inspector、Java は JVMのJDWP…）。だから1つ理解すれば横展開できます。

デバッガを理解するとき、**独立した2つの軸**で捉えると混乱しません。

**軸A：プロセスの主導権（launch か attach か）**

| | launch（起動型） | attach（接続型） |
|---|---|---|
| 誰がプログラムを起動する？ | IDE が起動する | 既に動いているプログラムに後から繋ぐ |
| 向いている対象 | ローカルのCLI・スクリプト | 常駐サーバー・**Webアプリ**・コンテナ |

Webアプリはリクエストがブラウザから来るので、**動いているPHPに繋ぐ attach 型**が自然です。

**軸B：接続の向き（どっちから繋ぎに行くか）**

```
■ 一般的な接続（例：Node）      IDE ──繋ぎに行く──▶ プログラム(ポートを開けて待つ)
■ Xdebug(DBGp) は逆向き        プログラム ──繋ぎに行く──▶ IDE(ポートを開けて待つ)
```

Xdebug は「attach 型 + 接続の向きが逆（プログラム→IDE）」という組み合わせです。

### だから設定ファイルはこう書く

「PHP側から IDE に電話をかけに行く」ので、設定は自然と
**「どこへ・いつ・どのファイルと対応づけて」発信するか**になります。

設定するファイルは次の2つです。

- **発信側（PHP/コンテナ）**：[docker/php/conf.d/xdebug.ini](../docker/php/conf.d/xdebug.ini)
- **待受側（IDE）**：`.vscode/launch.json`（各自のプロジェクト直下。作り方は [VSCode セットアップ](./vscode-setup.md)）

| 知りたいこと | 設定するファイル | 該当設定 |
|--------------|------------------|----------|
| **どこへ**発信する？（IDEの住所） | [docker/php/conf.d/xdebug.ini](../docker/php/conf.d/xdebug.ini)（発信側=PHP） | `client_host=host.docker.internal` / `client_port=9003` |
| **いつ**発信する？ | [docker/php/conf.d/xdebug.ini](../docker/php/conf.d/xdebug.ini) | `start_with_request=yes`（毎リクエスト） |
| どのポートで**待ち受ける**？ | `.vscode/launch.json`（待受側=IDE） | `"port": 9003`（xdebug.ini と一致させる） |
| コンテナ↔ローカルの**対応** | `.vscode/launch.json` | `pathMappings`（`/var/www/html` ↔ `src/`） |

- もし逆向き（IDE→プログラム）のデバッガなら、設定は真逆で「**プログラムが開くポート**」を
  指定します。Xdebug にはその設定は無く、代わりに上の「発信先・発信タイミング」があります。
- VS Code の `launch.json` は `"request": "launch"` と書きますが、**実際にPHPを起動はせず「待ち受ける」だけ**。
  名前が紛らわしいですが、逆向き接続の“着信待ち”です。
- 各設定の詳細は [3. xdebug.ini](#3-xdebugini--最重要の設定ファイル) と
  [VSCode セットアップ](./vscode-setup.md) を参照。

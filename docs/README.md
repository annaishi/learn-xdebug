...
4. **[③-2 基本操作を体で覚える](./03-basic-operations.md)**
   合計金額の計算ページ（`/playground`）で、ステップイン/オーバー/アウト・ループ・
   条件付きブレークポイント等を1つずつ体験する。最後に応用としてログイン処理を追う。
5. **[④ 演習：バグを Xdebug で直す](./04-exercise.md)**
   バグ入りの送料計算ページ（`/exercise`）を、調査 → 原因特定 → 修正 → 再確認する。
